...
class Video {
    ...
    public addShape(vertices: Array<number>) {
        let buffer = <WebGLBuffer>this.gl.createBuffer();
        this.gl.bindBuffer(this.gl.ARRAY_BUFFER, buffer);
        this.gl.bufferData(this.gl.ARRAY_BUFFER, new Float32Array(vertices), this.gl.STATIC_DRAW);
        let bufferIndex = this.buffers.push(buffer) - 1;
        let dShape = new Shape(bufferIndex, vertices.length);
        this.shapes.push(dShape);
        return dShape;
    }

    public cloneShape(shape:Shape) {
        let dShape = new Shape(shape.bufferIndex,shape.bufferSize);
        this.shapes.push(dShape);
        return dShape;
    }
    ...
    public addCircle(x: number, y: number, r: number) {
        ...
        let bufferIndex = this.buffers.push(buffer) - 1;
        let dShape = new Shape(bufferIndex, vertices.length);
        return this.shapes.push(dShape) - 1;
    }
        ...
        for (let i = 0; i < this.shapes.length; i++) {
            ...
            if (d.visible) {
                ...
                let matB = m3Multiply(matA, [
                    Math.cos(d.angle*Math.PI/180), -Math.sin(d.angle*Math.PI/180), 0,
                    Math.sin(d.angle*Math.PI/180), Math.cos(d.angle*Math.PI/180), 0,
                    0, 0, 1
                ]);
                ...
                this.gl.drawArrays(this.gl.LINES, 0, this.shapes[i].bufferSize / 2);
            }
